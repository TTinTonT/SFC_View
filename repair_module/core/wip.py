...
from repair_module.sql.wip_sql import WIP_GET_STATION_AND_NEXT


def get_station_and_next(conn, sn):
    """Return tuple: SN, MO, Model, STATION_NAME, LINE_NAME, GROUP_NAME, NEXT_STATION."""
    cur = conn.cursor()
    try:
...
def validate_next_station_r(next_station):
    """
    Validate next_station for Repair OK.
