# -*- coding: utf-8 -*-
"""
Repair actions: execute_repair_ok, check_has_unrepaired, get_group_info, jump_routing,
...
from repair_module.config.repair_config import REPAIR_ACTION_RECORD_MAP
from repair_module.sql.repair_sql import (
...
def check_has_unrepaired(conn, sn):
    """True if SN has R_REPAIR_T row with REPAIR_TIME IS NULL."""
    cur = conn.cursor()
    try:
...
def execute_repair_ok(
    ...
    """
    UPDATE R_REPAIR_T. Returns (rows_updated, success: bool, err_msg: str, repair_time).
    """
...
def get_group_info(conn, v_line, v_group):
    """Resolve LINE, SECTION, GROUP, STATION for target group."""
    cur = conn.cursor()
    try:
...
def get_jump_param_from_route(conn, sn, desired_target):
    """Find GROUP_NAME in C_ROUTE_CONTROL_T where GROUP_NEXT = desired_target."""
    cur = conn.cursor()
    try:
