...
def ro_next(sn: str, base: str, reason_code: str, remark: str = "", emp: str = ""):
    """Same as POST /api/debug/repair/ro-next."""
    ...
    if not reason_code:
        return {"ok": False, "error": "reason_code required for RO Next."}
    try:
        conn = get_conn()
        try:
            cur = conn.cursor()
            ...
            if not row or row[0] == 0:
                return {"ok": False, "error": "Invalid reason code for RO Next."}
            ...
            if get_dido_suffix_from_node(current_node) != "RO":
                return {"ok": False, "error": "SN must be at RO station for this action."}
            ...
            if not info:
                conn.rollback()
                return {"ok": False, "error": "GetGroupInfo not found for FLA; cannot jump."}
            ok = jump_routing(
                conn,
                sn,
                info["LINE_NAME"],
