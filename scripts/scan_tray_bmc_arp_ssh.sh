...
# Only scan these hostnames (DHCP leases)
ALLOW_HOSTS_REGEX="${ALLOW_HOSTS_REGEX:-bmc}"
EXCLUDE_HOSTS_REGEX="${EXCLUDE_HOSTS_REGEX:-dpu-bmc}"

# ARP + SSH settings
USE_ARP_SOURCE="${USE_ARP_SOURCE:-1}"
ARP_SUBNET_REGEX="${ARP_SUBNET_REGEX:-^172\\.16\\.}"
# USE_SSH_FRU: 0=ipmitool only, 1=SSH only, 2=hybrid (try ipmitool, fallback SSH)
USE_SSH_FRU="${USE_SSH_FRU:-2}"
...
arp_raw="$tmp_dir/arp_raw.tsv"
merged_best="$tmp_dir/merged_best.tsv"
...
[[ "$USE_SSH_FRU" == "1" || "$USE_SSH_FRU" == "2" ]] && command -v sshpass >/dev/null 2>&1 || { echo "ERROR: sshpass not found (required when USE_SSH_FRU=1 or 2). apt-get install sshpass"; exit 2; }

########################################
# Lightweight login test (0penBmc) - only when USE_SSH_FRU=0
########################################
ipmi_login_test() {
  local ip="$1"
...
########################################
# SSH FRU print (for BMCs where remote IPMI fails)
########################################
ssh_fru_print() {
  local ip="$1"
  sshpass -p "$BMC_PASS" ssh -o ConnectTimeout="$LOGIN_TIMEOUT" -o StrictHostKeyChecking=no "$BMC_USER@$ip" "ipmitool fru print" 2>/dev/null
}

########################################
# Parse FRU output: SN = Chassis Serial, PN = Chassis Part Number
# Supports Board Extra: MAC: and MAC1: (NVIDIA SMM_FRU)
########################################
parse_fru() {
  awk '
  ...
  /Board Extra[ \t]*:[ \t]*MAC[0-9]*:[ \t]*/{
    mac=$0; sub(/^.*Board Extra[ \t]*:[ \t]*MAC[0-9]*:[ \t]*/,"",mac); mac=trim(mac)
...
' "$LEASES_FILE" 2>/dev/null | sort -V > "$leases_raw"

########################################
# Dedup DHCP: keep latest by MAC, then by IP
########################################
awk 'BEGIN{FS=OFS="\t"}{mac=$1;ts=$4;if(!(mac in b)||ts>b[mac]){b[mac]=ts;l[mac]=$0}}END{for(m in l)print l[m]}' "$leases_raw" 2>/dev/null \
| awk 'BEGIN{FS=OFS="\t"}{ip=$2;ts=$4;if(!(ip in b)||ts>b[ip]){b[ip]=ts;l[ip]=$0}}END{for(i in l)print l[i]}' 2>/dev/null | sort -V > "$leases_best"

########################################
# ARP source: IPs from /proc/net/arp (subnet filter)
########################################
if [[ "$USE_ARP_SOURCE" == "1" ]]; then
  awk -v re="$ARP_SUBNET_REGEX" 'NR>1 && $3=="0x2" && $1~re && $4!="" && $4!="00:00:00:00:00:00" {
    mac=$4; gsub(/:/,"",mac); if(length(mac)==12) print mac "\t" $1 "\tarp-" $1 "\t0"
  }' /proc/net/arp 2>/dev/null | sort -u > "$arp_raw"
else
  : > "$arp_raw"
fi

########################################
# Merge DHCP + ARP: DHCP wins on IP conflict, dedup by IP
########################################
# leases_best: mac \t ip \t host \t ts
# arp_raw:     mac \t ip \t host \t 0
# merged: same format, DHCP entries first (higher ts), then ARP
{ cat "$leases_best" 2>/dev/null; cat "$arp_raw" 2>/dev/null; } \
| awk 'BEGIN{FS=OFS="\t"} {ip=$2;ts=$4+0;if(!(ip in seen)||ts>best[ip]){seen[ip]=1;best[ip]=ts;line[ip]=$0}} END{for(i in line)print line[i]}' \
| sort -V > "$merged_best"
...
########################################
# One worker: SSH FRU or IPMI FRU
########################################
run_one() {
  ...
  # Cache check first (skip FRU fetch if valid cache)
  local cached=""
  ...
  if [[ "$USE_SSH_FRU" == "1" ]]; then
    ...
    if ipmi_login_test "$ip" 2>/dev/null; then
      if [ "$(id -u)" -ne 0 ] && command -v sudo >/dev/null 2>&1; then
        ...
        fru_out="$(timeout "$FRU_TIMEOUT" ipmitool -I "$IPMI_IF" -C "$IPMI_CIPHER" -U "$BMC_USER" -P "$BMC_PASS" -H "$ip" -R "$IPMI_RETRY" -N "$IPMI_RETRANS" fru print 2> "$errfile")" || true
      fi
    fi
  ...
  if [[ -z "$fru_out" || "$fru_out" != *"FRU Device Description"* ]]; then
    ...
    echo -e "$ip\tNA\tNA\tNA\tNA\tFAIL\t${fru_err:-fru failed}"
    return
  fi
...
export -f run_one parse_fru ipmi_login_test ipmi_fru_print ssh_fru_print
export tmp_dir FAILURES_FILE new_failures new_cache CACHE_FILE CACHE_TTL now_ts
export LOGIN_TIMEOUT FRU_TIMEOUT USE_SSH_FRU
