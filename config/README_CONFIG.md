...
| CRABBER_SITENAME                                                                   | `sitename` for thread quota check (San Jose Crabber)                                                                                     | `SanJose`                                                              |
| CRABBER_SV_BASE_URL                                                                | Crabber API base when user selects **SV** (Online Test / Testing overview)                                                               | `http://10.24.10.174:8000`                                             |
| CRABBER_SV_TOKEN                                                                   | API token for SV; if empty, `CRABBER_TOKEN` is used (often rejected on SV — set to the SV UI `Authorization: Token …` value)              |                                                                        |
| CRABBER_SV_USER_ID                                                                 | Crabber `user_id` query/body for SV Online Test; if empty, `CRABBER_USER_ID` is used (SV UI may use e.g. `12` vs SJ `41`)                 |                                                                        |
| CRABBER_SV_SITENAME                                                                | `sitename` for SV thread quota (`check_is_over_thread_quota`); must match SV Crabber (e.g. `SV_Worker4`)                                 | `SV_Worker4`                                                           |
| **UI**                                                                             | Browsers send `crabber_profile=sj` (default) or `crabber_profile=sv` on Online Test and Testing overview APIs                            |                                                                        |
| **ETF**                                                                            |                                                                                                                                          |                                                                        |
| SFC_TRAY_STATUS_URL                                                                | Tray status API (**SJ MTF** on `/debug/l10-test`; same endpoint as SJ `…/testdashboard/L10.html`)                                             | `http://10.16.137.115/SFCAPI/SFC/Test_Fixture_Status`                 |
| SFC_TRAY_STATUS_URL_SV                                                             | Tray status POST URL for **SV MTF** (same JSON as SJ; empty disables SV tray fetch; mirrors `…/testdashboard_SV/L10.html`)               | `http://10.16.137.115/SFCAPI_SV/SFC/Test_Fixture_Status`               |
